...
class Robot(Node):
	def __init__(self):
		...
		self.FL_tick_sub = self.create_subscription(Int32, 'FL_tick', self.FL_tick_callback, 10)
		self.FR_tick_sub = self.create_subscription(Int32, 'FR_tick', self.FR_tick_callback, 10)
	...
	def odometry(self):
		...
		dt = dt.nanoseconds / 1e9 # sec
  
		if tick['FL'] == None or tick['FR'] == None:
			return

		if self.FL_tick['bf'] == None or self.FR_tick['bf'] == None:
		...
		DPT = (2.0 * math.pi * R) / TPR # distance per tick
